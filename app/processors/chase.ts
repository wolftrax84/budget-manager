...
export const process = async (file: string, accountId: string): Promise<Array<[string, Expense | Transfer]>> => {
        ...
        const transaction =
            parts[4] === 'Payment'
                ...
                : ({
                      ...
                      amount,
                      vendorId,
                      category: vendorId ? (await getVendor(vendorId)).category || "" : "",
                      subcategory: vendorId ? (await getVendor(vendorId)).subcategory || "" : "",
                      kind: 'expense',
                  } satisfies Expense)
